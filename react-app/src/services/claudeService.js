...
const CLAUDE_API_URL = import.meta.env.DEV ? "/api/claude" : "/api/claude";
  ...
  try {
    if (!CLAUDE_API_KEY || CLAUDE_API_KEY.includes("sua_chave")) {
      return {
        success: false,
        error:
          "API Key do Claude não configurada. Adicione VITE_ANTHROPICSK_API_KEY no arquivo .env",
      };
    }
    ...
    if (!avisoData) {
      if (textResponse.includes("{") && textResponse.includes("}")) {
        ...
        return {
          success: false,
          error:
            "Não foi possível processar a resposta da IA. Tente reformular sua mensagem.",
        };
      }
      return {
        success: true,
