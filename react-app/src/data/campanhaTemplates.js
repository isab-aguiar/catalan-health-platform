...
// =========================================
// Layouts governamentais pré-definidos para campanhas visuais
...
export const campanhaTemplates = {
  ...
  vacinacao: {
    ...
    layout: {
      tipo: 'destaque-hero',
      cores: {
        primaria: '#1d4ed8', // blue-700
        secundaria: '#3b82f6', // blue-500
    ...
    estilo: {
      container: 'bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-300 rounded-2xl shadow-lg overflow-hidden',
      header: 'bg-blue-700 text-white p-6',
      badge: 'bg-blue-500 text-white px-4 py-2 rounded-full text-sm font-semibold',
  ...
  material: {
    ...
    estilo: {
      container: 'bg-red-50 border-l-4 border-red-600 rounded-lg shadow-md overflow-hidden',
      header: 'bg-red-600 text-white p-4 flex items-center gap-3',
      badge: 'bg-red-500 text-white px-3 py-1 rounded-full text-xs font-semibold',
      icone: 'w-8 h-8',
      titulo: 'text-2xl font-bold text-white',
      corpo: 'p-6 bg-white',
      descricao: 'text-slate-700 text-base leading-relaxed',
      dataValidade: 'text-sm text-slate-500 mt-4 border-t border-slate-200 pt-4'
    }
  },
  ...
  educacao: {
    ...
    estilo: {
      container: 'bg-gradient-to-br from-green-50 to-emerald-50 border border-green-300 rounded-2xl shadow-lg overflow-hidden',
      header: 'relative',
      imagem: 'w-full h-72 object-cover',
      overlay: 'absolute inset-0 bg-gradient-to-t from-green-900/90 to-transparent',
      badge: 'absolute top-4 left-4 bg-green-600 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg',
      titulo: 'absolute bottom-6 left-6 text-4xl font-bold text-white drop-shadow-lg',
      corpo: 'p-6 bg-white',
  ...
  urgente: {
    ...
    estilo: {
      container: 'bg-gradient-to-br from-red-50 via-orange-50 to-amber-50 border-2 border-red-400 rounded-xl shadow-xl overflow-hidden animate-pulse-slow',
      header: 'bg-gradient-to-r from-red-600 to-orange-600 text-white p-6 flex items-center gap-4',
      badge: 'bg-amber-500 text-white px-4 py-2 rounded-full text-sm font-bold uppercase tracking-wide',
  ...
  evento: {
    ...
    estilo: {
      container: 'bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-300 rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300',
      imagem: 'w-full h-56 object-cover',
      header: 'p-6 bg-white',
      badge: 'bg-violet-600 text-white px-3 py-1 rounded-full text-xs font-semibold inline-block mb-3',
      ...
      descricao: 'text-slate-700 text-base leading-relaxed mb-4',
      inscricao: 'bg-violet-600 hover:bg-violet-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 w-full text-center'
    }
  },
  ...
  informativo: {
    ...
    estilo: {
      container: 'bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden',
...
// Função para selecionar template baseado na categoria e contexto
export const selecionarTemplate = (categoria, urgente = false, temImagem = false) => {
  // Se for urgente, retorna o template urgente
  if (urgente) {
    return campanhaTemplates.urgente;
  }

  // Seleção baseada na categoria
  switch (categoria?.toLowerCase()) {
    ...
    default:
      // Se tem imagem, usa informativo com imagem
      // Senão, usa informativo simples
      return temImagem ? campanhaTemplates.informativo : campanhaTemplates.informativo;
  }
};

// Função para gerar prompt para a IA baseado no template
export const gerarPromptTemplate = (template, arquivoInfo) => {
  const { nome, categoria, layout } = template;
  
  return `
Você está criando uma campanha para UBS São José usando o template "${nome}".
