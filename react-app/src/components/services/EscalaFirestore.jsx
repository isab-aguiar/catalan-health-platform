 ...
 * Busca dados do Firestore (escala mensal atual OU escala padrão)
 * Suporta 3 turnos: manhã, tarde e Saúde na Hora
 */
export default function EscalaFirestore({
  titulo = "Profissionais Escalados",
  escalaKey // chave da escala no Firestore (ex: "sala-procedimentos")
}) {
  const [escala, setEscala] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [tipoEscala, setTipoEscala] = useState(null); // "mensal" ou "padrao"
  ...
  // Verificar se escala semanal está habilitada
  const escalaSemanalHabilitada = escala.escalaSemanal?.habilitado === true;
  
  // Verificar se há profissionais (modo normal ou semanal)
  const temProfissionaisNormais = escala.profissionais && escala.profissionais.length > 0;
  const temProfissionaisSemanais = escalaSemanalHabilitada && 
  ...
  // Renderizar escalas por turno (apenas para modo normal)
  ...
  // Função para agrupar profissionais por dias da semana
  const agruparProfissionaisPorDias = (escala) => {
    ...
    // Coletar todos os profissionais únicos com seus dias
    ...
    // Separar em "todos os dias" e "dias específicos"
    const resultado = {
      todosOsDias: [], // Profissionais em segunda a sexta (apenas manhã/tarde)
      diasEspecificos: {} // { dia: [profissionais] }
    };

    profissionaisUnicos.forEach((prof) => {
      // Saúde na Hora (noite) sempre aparece em dias específicos, nunca agrupado como "Segunda a Sexta"
      const isSaudeNaHora = prof.turno === 'saudeNaHora';
      
      if (prof.dias.size === 5 && !isSaudeNaHora) {
        // Está em todos os dias E não é "saúde na hora"
        resultado.todosOsDias.push({
          ...prof,
          dias: undefined // Remover Set antes de usar
        });
      } else {
        // Está em dias específicos OU é "saúde na hora"
    ...
    <div className="bg-neutral-50 border border-neutral-200 rounded-lg p-5">
      ...
      {(escala.horarios?.manha?.ativo || escala.horarios?.tarde?.ativo || escala.horarios?.saudeNaHora?.ativo) && (
        <div className="mb-5 bg-gradient-to-r from-blue-50 to-teal-50 border-l-4 border-blue-600 rounded-lg p-4 shadow-sm">
          ...
          {/* Determinar dias da semana */}
          {(() => {
            const diasAtivos = new Set();
            const diasKeys = ['segunda', 'terca', 'quarta', 'quinta', 'sexta'];
            
            if (escalaSemanalHabilitada && escala.escalaSemanal?.dias) {
              // Verificar todos os dias que têm profissionais ativos na escala semanal
              diasKeys.forEach(dia => {
                const diaData = escala.escalaSemanal.dias[dia];
                // Verificar se o dia está ativo E tem profissionais
                if (diaData && diaData.ativo === true) {
                  // Verificar se tem profissionais no array
                  if (diaData.profissionais && Array.isArray(diaData.profissionais) && diaData.profissionais.length > 0) {
                    diasAtivos.add(dia);
                  }
                }
              });
            } else {
              // Quando escala semanal não está habilitada, verificar observações para determinar dias
              // ou assumir segunda a sexta como padrão (mas não mostrar se não houver indicação)
              if (escala.observacoes && Array.isArray(escala.observacoes)) {
                const observacoesTexto = escala.observacoes.join(' ').toLowerCase();
                
                // Verificar padrões comuns nas observações
                if (observacoesTexto.includes('quarta') || observacoesTexto.includes('quartas')) {
                  diasAtivos.add('quarta');
                }
                if (observacoesTexto.includes('segunda') && !observacoesTexto.includes('não')) {
                  diasAtivos.add('segunda');
                }
                if (observacoesTexto.includes('terça') && !observacoesTexto.includes('não')) {
                  diasAtivos.add('terca');
                }
                if (observacoesTexto.includes('quinta') && !observacoesTexto.includes('não')) {
                  diasAtivos.add('quinta');
                }
                if (observacoesTexto.includes('sexta') && !observacoesTexto.includes('não')) {
                  diasAtivos.add('sexta');
                }
                
                // Se mencionar "segunda a sexta" ou "todos os dias"
                if (observacoesTexto.includes('segunda a sexta') || observacoesTexto.includes('todos os dias')) {
                  diasKeys.forEach(dia => diasAtivos.add(dia));
                }
              }
            }
            
            // Se não há dias ativos, não mostrar badge
            ...
            if (diasAtivos.size === 5) {
              ...
              // Ordenar os dias para manter a ordem correta
              const diasOrdenados = Array.from(diasAtivos).sort((a, b) => {
                const ordem = { segunda: 1, terca: 2, quarta: 3, quinta: 4, sexta: 5 };
      ...
      {escalaSemanalHabilitada ? (
        /* Modo Escala Semanal - Agrupado por Dia com lógica de agrupamento */
        (() => {
          ...
          return (
            <>
              {/* Profissionais que atendem Segunda a Sexta */}
                        ...
                        {todosOsDias.map((prof, idx) => (
                          <tr key={idx} className="bg-white">
                            <td className="border border-neutral-300 px-4 py-2">
                              <strong className="text-neutral-800 text-sm">{prof.nome}</strong>
                            </td>
                            ...
                            <td className="border border-neutral-300 px-4 py-2 text-neutral-700">
                              <div className="space-y-1">
                                ...
                                {escala.horarios?.saudeNaHora?.ativo && prof.turno === 'saudeNaHora' && (
                                  <div className="flex items-center gap-2">
                                    <Clock className="w-3 h-3 text-green-600" />
                                    <span className="text-xs">Saúde na Hora: {escala.horarios.saudeNaHora.display}</span>
                                  </div>
                                )}
                              </div>
                            </td>
              ...
              {/* Profissionais que atendem em dias específicos */}
        ...
        /* Modo Normal - Todos os Profissionais Juntos */
        <>
          {/* Versão Desktop */}
          <div className="hidden md:block">
            <table className="w-full border-collapse border border-neutral-300">
